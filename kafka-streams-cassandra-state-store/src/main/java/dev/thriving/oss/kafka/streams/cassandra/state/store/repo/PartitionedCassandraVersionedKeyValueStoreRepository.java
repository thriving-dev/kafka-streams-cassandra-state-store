...
public class PartitionedCassandraVersionedKeyValueStoreRepository<K>
        extends AbstractCassandraStateStoreRepository<K>
        implements CassandraVersionedKeyValueStoreRepository {

    private static final Logger LOG = LoggerFactory.getLogger(PartitionedCassandraVersionedKeyValueStoreRepository.class);
    ...
    public PartitionedCassandraVersionedKeyValueStoreRepository(CqlSession session,
                                                                String tableName,
                                                                boolean createTable,
                                                                String tableOptions,
    ...
    protected void initPreparedStatements(String tableName) {
        selectByPartitionAndKeyAndValidTo = session.prepare("SELECT value, time, validFrom, validTo FROM " + tableName + " WHERE partition=? AND key=? AND validTo=?");
        selectByPartitionAndKeyAndValidToGTE = session.prepare("SELECT value, time, validFrom, validTo FROM " + tableName + " WHERE partition=? AND key=? AND validTo>? ORDER BY validTo ASC LIMIT 1");
        cleanup = session.prepare("DELETE FROM " + tableName + " WHERE partition=? AND key=? AND validTo<? AND validTo>=0");
        insert = session.prepare("INSERT INTO " + tableName + " (partition, key, validFrom, validTo, time, value) VALUES (?, ?, ?, ?, ?, ?)");
    }
    ...
    public void saveInBatch(int partition, Bytes key, List<VersionedEntry> entries) {
        ...
        if (entries.size() == 1) {
            ...
            BoundStatement stmt = insert.bind(partition,
                    ByteBuffer.wrap(key.get()),
                    entry.validFrom(),
                    entry.validTo(),
                    entry.timestamp(),
